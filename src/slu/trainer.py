...
class SLUTrainer(object):
    ...
    def train_step(self, X, lengths, y_bin, y_final, y_dm, templates=None, tem_lengths=None, epoch=None):
        ...
        if self.params.use_contrastive_slotname_predictor:
            ...
            loss_slotname, slot_contrastive_loss, context_contrastive_loss = self.slotname_predictor(y_dm, lstm_hiddens, lengths = lengths, epoch=epoch,binary_golds=y_bin, final_golds=y_final)
        else:
            loss_slotname = self.slotname_predictor(y_dm, lstm_hiddens, binary_golds=y_bin, final_golds=y_final)
        ...
        if self.use_label_encoder:
            return loss_bin.item(), loss_total.item(), template0_loss.item(), template1_loss.item()
        else:
            if context_contrastive_loss is not None:
                return loss_bin.item(), loss_slotname.item(), slot_contrastive_loss.item(), context_contrastive_loss.item()
            else:
                return loss_bin.item(), loss_slotname.item(), slot_contrastive_loss.item(), 0.0

    
    def evaluate(self,params, dataloader, istestset=False,test_tag=True):
        ...
        for i, (X, lengths, y_bin, y_final, y_dm) in pbar:
            ...
            slotname_preds_batch = self.slotname_predictor(y_dm, lstm_hiddens, binary_preditions=bin_preds_batch, binary_golds=None, final_golds=None, lengths=lengths)
            final_preds_batch, final_pred_logits_batch = self.combine_binary_and_slotname_preds(y_dm, bin_preds_batch, slotname_preds_batch)
        ...
        if test_tag:
            print("best f1 score util now:",self.best_f1)
        ...
        for i in range(len(dm_id_batch)):
            ...
            for bin_pred in binary_preds:
                ...
                if bin_pred.item() == 0:
                    ...
                    preds_logits = {slot_list_based_dm[ind]:round(x.item(),2) for ind,x in enumerate(slotname_preds[i])}
                    final_preds_logits_each.append(preds_logits)
                    ...
                    if i == -1:
                        ...
                        # final_preds_logits = {slot_list_based_dm[ind]:round(x.item(),2) for ind,x in enumerate(slotname_preds[i])}
                        # final_preds_logits_each.append(final_preds_logits)
            ...
            final_preds_logits.append(final_preds_logits_each)
            
        return final_preds, final_preds_logits
